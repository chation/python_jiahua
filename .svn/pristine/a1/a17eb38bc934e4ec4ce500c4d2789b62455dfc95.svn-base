...
class CallbackConf(object):
    ...
    def __new__(cls, *args, **kw):
        if not hasattr(cls, '_instance'):
            orig = super(CallbackConf, cls)
            cls._instance = orig.__new__(cls, *args, **kw)
        return cls._instance

    def __init__(self,token=None,encodingAESKey=None,corpId=None):
        '''
        ...
        self.token = token if token else config.TOKEN
        self.encoding_aes_key = encodingAESKey if encodingAESKey else config.ENCODING_AES_KEY
        self.corpid = corpId if corpId else config.CORPID
...
class ActiveConf():
    ...
    def grant_access_token(self):
        ...
        print response_json
        if (response_json['errcode'] == -1)or(response_json['errcode']== 1235):
