...
def createCat(request):
    if request.method == "POST":
        '''
        validateResult = commonLib.validateField(request,"plateNum","catType","fullName","phoneNumber","catOther")
        if not validateResult[0]:
            return commonLib.statusJson(status=401,message=validateResult[1])        
        '''
        cat = Cat.create(request.POST)
        #数据验证
        try:
            cat.full_clean()
        except ValidationError as e:
            return commonLib.statusJson(status=402,body=e)
...
#查询车辆
def queryCat(request):
    if request.method == "POST":
        #armyQuerySet = Army.objects.all()
        #armyList  = serializers.serialize("python",armyQuerySet)
        catQuerySet = Cat.objects.values("id","plateNum","catType","fullName","phoneNumber","catOther","ofterPlace")
        catList = list(catQuerySet)
        return commonLib.statusJson(body=catList)
